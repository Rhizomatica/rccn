...
class Billing:
	...
	def get_rate(self, destination_number):
		# identify which logger to use
		frame = sys._getframe(2)
		#print 'Calling: %s' % frame.f_code.co_name
	...
	def bill(self,session,subscriber,destination_number,context,duration):
		...
		if context == 'INBOUND':
			...
			print bleg_connected
			print hangup_cause
			print subscriber
			print session.getVariable('aleg_duration')
			print session.getVariable('bleg_duration')
			...
			if (bleg_connected != '' and bleg_connected != None) and hangup_cause == 'NORMAL_CLEARING':
				...
				try:
					...
					if charge_info[1] == 'call':
						...
						#duration = session.getVariable('billsec')
