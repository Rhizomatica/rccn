...
class Reseller:
	...
	def edit(self, msisdn, pin, balance):
                try:
                        cur = db_conn.cursor()
                        if balance != '':
                                cur.execute('UPDATE resellers SET msisdn=%(msisdn)s,pin=%(pin)s,balance=%(balance)s WHERE msisdn=%(msisdn2)s', {'msisdn': msisdn, 'name': name, 'balance': Decimal(str(balance)), 'msisdn2': msisdn})
                        else:
	...
	def bill(self, amount):
		try:
			cur = db_conn.cursor()
			cur.execute('UPDATE resellers SET balance=%(balance)s, total_sales = total_sales + 1 WHERE msisdn = %(msisdn)s', {'balance': Decimal(str(self.balance)), 'msisdn': self.reseller_msisdn})
		except psycopg2.DatabaseError as e:
			db_conn.rollback()
			raise ResellerException('Error in setting Reseller balance')
		
		try:
			cur.execute('INSERT INTO resellers_credit_history(reseller_msisdn,previous_balance,current_balance) VALUES(%(r_msisdn)s,%(prev_balance)s,%(curr_balance)s)', {'r_msisdn': self.reseller_msisdn, 'prev_balance': Decimal(str(self.previous_balance)), 'curr_balance': Decimal(str(self.balance))})
		except psycopg2.DatabaseError as e:
			db_conn.rollback()
			raise ResellerException('Error creating invoice for reseller')

		try:
			cur.execute('INSERT INTO resellers_transactions(reseller_msisdn,subscriber_msisdn,amount) values(%(r_msisdn)s,%(s_msisdn)s,%(amount)s)', {'r_msisdn': self.reseller_msisdn, 's_msisdn': self.subscriber_msisdn, 'amount': Decimal(str(amount))})
		except psycopg2.DatabaseError as e:
			db_conn.rollback()
