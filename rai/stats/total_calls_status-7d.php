...
$stmt = $dbh->query("select strftime('%d-%m-%Y',date(calldate)),count(*),disposition from cdr where calldate > (select datetime('now', '-7 day')) group by date(calldate),disposition");
...
// Setup the titles
$graph->title->Set("Last 7 days - Total daily calls breakdown by status");
