...
try:
    ...
    for item in msgs:
        ...
        coding = item['coding']
        n = n + 1
        sms_log.info ("Message ID: %s for %s [ %s ]" % (mid, dest, utext))
        try:
        ...
        if c_bts != vpn_ip_address:
            sms_log.info("Found an SMS with ID %s (%s) for %s but they are at %s" % (s[0], mid, dest, c_bts))
            sms_log.info("Re-Injecting [%s]...." % utext.encode('utf-8','replace') )
            try:
                sms_log.info("Sending Message directly to RAPI on %s" % c_bts)
                result = sms.send(src, dest, utext, 'unicode', c_bts)
                sms_log.info("Result from SMS.send(): %s" % result)
            ...
            sms_log.info("Destination Number is here according to HLR")
