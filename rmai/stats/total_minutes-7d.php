...
$stmt = $dbh->query("select strftime('%d-%m-%Y',date(calldate)),sum(billsec)/60 from cdr where calldate > (select datetime('now', '-7 day')) group by date(calldate)");
$datax = array();
$datay = array();
...
// Setup the titles
$graph->title->Set("Last 7 days - Total calls minutes");
