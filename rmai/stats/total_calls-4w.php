...
$stmt = $dbh->query("select * from  (select strftime('%W',calldate) calldate,count(*) from cdr group by strftime('%W', calldate) order by strftime('%W',calldate)  DESC limit 7 ) q order by calldate asc");
$datax = array();
$datay = array();
...
// Setup the titles
$graph->title->Set("Last 4 weeks - Total calls per week");
