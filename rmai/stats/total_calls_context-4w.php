...
$stmt = $dbh->query("select * from (select strftime('%d-%m-%Y',date(calldate)) calldate ,count(*),dcontext from cdr where calldate > (select datetime('now', '-7 day')) group by date(calldate),dcontext order by strftime('%W',calldate)  DESC limit 12 ) q order by calldate asc");
